...
같은 주제로 **개발 방향을 백 번 넘게** 갈아엎었습니다. 만들고, 지우고, 다시 올렸습니다. 지표는 맞는 것 같았습니다. 룰도, 알고리즘도 그럴듯했습니다. 그런데 막상 시장과 맞닿으면 자꾸 어긋났습니다.

수치가 틀린 건 아니었던 것 같습니다. 그런데 숫자만으로는 부족했습니다. float이 작아도 세션이 다르면 이야기가 달라집니다. 뉴스가 비슷해도 공매도 구조가 다르면 방향이 갈립니다. **데이터와 데이터 사이**에 깔리는 장면—한 줄의 규칙으로는 다 담기 어려운 맥락이 빠지면, 같은 숫자도 다른 말을 합니다.

그래서 과거에 실제로 벌어진 사례와, 지금 쓰는 시세 API가 무엇을 의미하는지에 대한 설명을 같은 자리에 두고, 여러 에이전트가 그 위에서 판단하게 하는 백엔드를 만들었습니다.
